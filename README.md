...
Another improvement would be to first use a parser and tokenize the code in order to locate all the uses of `sg` in the text in order to extract the specific calls to **PySimpleGUI**. This would be the most efficient way for handling **all calls** to **PySimpleGUI**. I did not look into parsing the code, but might do this in the future.
...
It is very easy to fix such breaks: just add the missing item to the respective tuples, either `empty_func_list`, `empty_consts_list `, or `elements_list`.
